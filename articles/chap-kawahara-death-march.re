...
ストーリーを作成することにより、ゲーム全体の世界観や最終的な工数が見えてきます、ストーリーを作成しているとき、プログラムの実装に入っており、企画者にフィードバックすることもあります。

ストーリー作成中にプロットとは違うことをすること、たえばイベント間のつながり変更することは、プログラムの変更を意味し見積もりが大幅に変わります、それと同時に工数が増えます。
...
株主を含む立場の強い企画者は、積み上げてきた物をバベルの塔を破戒@<fn>{メテオフォール型開発}するがのごとく、見積もりを含む全てのシステムを無にします。
このケースでは年単位の遅れも発生します。

//footnote[メテオフォール型開発][ゲーム製作時代はバベルの塔を破戒されたと言ってましたが、最近では「メテオフォール型開発」@<href>{http://eiki.hatenablog.jp/entry/meteo_fall} という言葉が生まれました。]
