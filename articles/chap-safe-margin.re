...
工数を見積もる際にはまだ作業していないのに時間を予測しますから、完璧な精度で算出することは不可能ですよね。
実際の作業時間がその見積もりを超過すると納期遅延など大きな問題に発展する可能性がありますので、私たちはそうならないよう細心の注意を払い「実工数＜見積もり工数」となるよう調整します。
見積もりの精度を上げるには、一般的には作業を細分化して過去の実績など経験と勘をもとに算定しますが、それでも想定しきれない諸々の問題に対処するために余裕時間「安全マージン」を積み増します。
どの程度の安全マージンを設定するかは場面によって大きく変わりますし、設定量によっては説明が難しくなる場合もあるため、難しい課題ではないでしょうか。
本章ではこの安全マージンについて私の体験談を元に解説し、特に@<br>{締切稼働}の作業にならないためのコツを工数見積もり初心者に向けて共有します。
...
何においても「初めて」は想定しきれない部分が多くリスクを伴いますよね。
また「初めての顧客」についても、相手方の協力姿勢や技術理解度などを推し量る事ができないため、リスク因子となり得ます。
...
こういった未知（初めて）の事象に対しては十分な余裕を持ち取り組む必要があります。
...
その他にも「まだ決まっていない」「後で決めればいい」のように先延ばしにしがちな部分があれば大きなリスクになりますよね。
...
この場合には相当のコミュニケーションコストがかかることが想定されます。
例えばオフショア開発チームと日本語でのコミュニケーションが困難な場合に何回も手戻りが発生し、工数が膨れ上がるといった可能性を考慮するべきです。
...
「安全マージン率をどうするか」というと理解しづらいかもしれませんので、例えば1日あたりの作業時間を6～7時間のように余裕をもたせるのが良いでしょう。
